...
def overtime_minutes(minutes: int) -> int:
    return max(minutes - STANDARD_MINUTES, 0)
...
"""
def format_line(name: str, stat: dict) -> str:
    line = (f'{name}: {format_hours(stat["minutes"])}'
            f' / 最長 {format_hours(stat["longest"])}'
            f' / 残業 {format_hours(stat["overtime"])}'
    )

    if stat["missing"] > 0:
        line += f' / 要確認 {stat["missing"]}件（{", ".join(stat["days"])}）'
...
def format_line(person: PersonSummary) -> str:
    ...
    line = (f'{person.name}: {format_hours(person.minutes)}'
                f' / 最長 {format_hours(person.longest)}'
                f' / 残業 {format_hours(person.overtime)}'
        )
    
    if person.missing > 0:
        line += f' / 要確認 {person.missing}件（{", ".join(person.missing_days)}）'
